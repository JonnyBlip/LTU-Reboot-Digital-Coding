 ...
 * @param {obj} objCoupon TA coupon object.
...
function getTotalPrice(arrShoppingCart=[],objCoupon={}){
  ...
  let total = 0;
  //setting the item price to 0 so it isnt redeclared in the loop
  let itemPrice = 0;
  //loops through each item in the shopping cart array
  for(var counter=0; counter < arrShoppingCart.length; counter++){
    // console.log(counter);
    //selects the current cart item using the counter as the array key
